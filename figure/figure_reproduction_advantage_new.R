...
advantagedata <- lapply(1:nsample, function(x) {
  ...
  gen3 <- diff(pgamma(seq(0, 14, by=0.5), 1/gkappa_delta_orig, 1/gkappa_delta_orig/gbar_delta_orig))
  ...
  R_delta_pearson <- tail(i1, -n)/sapply(1:(length(i1)-n), function(y) sum(i1[y:(y+n-1)] * rev(gen3)))
  R_omicron_pearson <- tail(i2, -n)/sapply(1:(length(i2)-n), function(y) sum(i2[y:(y+n-1)] * rev(gen3)))
  ...
  data.frame(
    ...
    R_delta_pearson=R_delta_pearson,
    R_omicron_pearson=R_omicron_pearson,
    R_advantage_pearson=R_omicron_pearson/R_delta_pearson,
    r_delta=tail(gfit1_post[,x], -n),
    r_omicron=tail(gfit2_post[,x], -n),
    r_advantage=tail(advantage_post[,x], -n),
    sim=x,
    time=tail(seq(1, 10, by=1/14), -n),
    bias_naive=R_omicron_naive/R_delta/(R_omicron/R_delta),
    bias_pearson=R_omicron_pearson/R_delta_pearson/(R_omicron/R_delta)
  )
}) %>%
  bind_rows
...
g3 <- ggplot(filter(advantagesumm, key %in% c("R_advantage_pearson", "R_delta_pearson", "R_omicron_pearson"))) +
  geom_hline(yintercept=1, lty=2) +
  geom_ribbon(aes(time, ymin=lwr, ymax=upr, fill=key), alpha=0.2) +
  geom_line(aes(time, median, col=key), lwd=0.8) +
  annotate("text", x=Inf, y=Inf, label="Identical generation-interval distributions \n Pearson et al. (2022)", hjust=1.01, vjust=1.3, family="Times") +
  scale_x_continuous("Date", breaks=2:10,
                     labels=c("Nov 28", "Dec 5", "Dec 12", "Dec 19", "Dec 26", "Jan 2", "Jan 9", "Jan 16",
                              "Jan 23")) +
  scale_y_log10("Reproduction number", breaks=c(0.5, 1, 2, 4), limits=c(0.5, 4), expand=c(0, 0)) +
...
g4 <- ggplot(filter(advantagesumm, key %in% c("bias_pearson", "bias_naive"))) +
  geom_hline(yintercept=1, lty=2) +
  geom_ribbon(aes(time, ymin=lwr, ymax=upr, fill=key), alpha=0.2) +
  geom_line(aes(time, median, col=key), lwd=0.8) +
  scale_x_continuous("Date", breaks=2:10,
  ...
  scale_color_discrete(labels=c("Naive", "Pearson et al. (2022)")) +
  scale_fill_discrete(labels=c("Naive", "Pearson et al. (2022)")) +
  theme(
    panel.grid = element_blank(),
    legend.title = element_blank(),
    legend.position = "right"
...
ggsave("figure_reproduction_advantage_new.pdf", gfinal, width=12, height=4)

ggsave("figure_reproduction_advantage_new_bias.pdf", g4, width=6, height=4)

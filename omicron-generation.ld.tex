...
%DIF DEL omicron-generation.tex.933813a3.oldfile   Sun Apr  2 09:52:36 2023
%DIF ADD omicron-generation.tex                    Sun Apr  2 09:50:40 2023
...
Estimating transmission advantages of new SARS-CoV-2 variants is critical to predicting and controlling the course of the COVID-19 pandemic \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{park2021roles}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{park2022importance}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\DIFaddbegin \DIFadd{Overall, our revised estimates are more consistent with contact tracing studies that reported 1.5--2.2 fold increase in secondary attack rates in household and non-household settings \mbox{%DIFAUXCMD
\citep{allen2022comparative,del2022secondary}}\hspace{0pt}%DIFAUXCMD
.
}\DIFaddend 
...
We \DIFaddbegin \DIFadd{assume that incubation periods and generation intervals follow a bivariate lognormal distribution---thus, we implicitly assume a power-law relationship between them. 
This was done because data were limited, and previous work \mbox{%DIFAUXCMD
\cite{sender2021unmitigated} }\hspace{0pt}%DIFAUXCMD
has shown that this approach can also capture important aspects of the relationship.
In fact, however, previous studies have shown that mechanistic models capture this relationship better \mbox{%DIFAUXCMD
\citep{ferretti2020timing,hart2021}}\hspace{0pt}%DIFAUXCMD
. 
When sufficient data are available, more mechanistic approaches may therefore be more suitable. 
}

\DIFadd{We }\DIFaddend do not account for individual-level heterogeneity, such as age, vaccination status, or previous exposure history.
...
These conclusions are relevant for other emerging and endemic pathogens in general\DIFaddbegin \DIFadd{.
}

\DIFadd{This study also has potential implications for studying generation times in population biology \mbox{%DIFAUXCMD
\citep{leslie1966intrinsic,mertz1971mathematical,caswell2009stage,steiner2014generation}}\hspace{0pt}%DIFAUXCMD
.
...
Their difference are therefore likely affected by dynamical biases \mbox{%DIFAUXCMD
\citep{jonasson2022unifying}}\hspace{0pt}%DIFAUXCMD
.
Similarly, the connection between generation times based at different recruitment stages is analogous to that between serial and generation intervals \mbox{%DIFAUXCMD
\citep{lebreton2005age,bienvenu2015new,giaimo2023generation}}\hspace{0pt}%DIFAUXCMD
.
Comparing definitions and methods across these fields could provide valuable future insights}\DIFaddend .

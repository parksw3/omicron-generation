...
fit_he_r_50_nsgtf_within <- lapply(r_nsgtf, function(r) {
  print(r)
  ...
  ff <- fitfun_he(data=data_50_nsgtf_within, 
                  inf_shape=coef(fit_he_base_50_nsgtf_within)[[1]], 
                  inf_mean=coef(fit_he_base_50_nsgtf_within)[[2]], 
                  inf_shift=5,
                  logmean_inc=logmean_inc_nsgtf, 
                  logsd_inc=logsd_inc_nsgtf, 
                  r=r)
  
  mean <- mean_inc_nsgtf - 5 + coef(ff)[[2]]
  
  cc <- confint(ff, method="quad")
  
  data.frame(
    r=r,
    param=c("inf_shape", "inf_mean", "mean"),
    est=c(coef(ff)[[1]], coef(ff)[[2]], mean),
    lwr=c(cc[1,1], cc[2,1], mean_inc_nsgtf - 5 + cc[2,1]),
    upr=c(cc[1,2], cc[2,2], mean_inc_nsgtf - 5 + cc[2,2]),
    type="Delta"
  )
}) %>%
  bind_rows

fit_he_r_50_sgtf_within <- lapply(r_sgtf, function(r) {
  print(r)
  ...
  ff <- fitfun_he(data=data_50_sgtf_within, 
                  inf_shape=coef(fit_he_base_50_sgtf_within)[[1]], 
                  inf_mean=coef(fit_he_base_50_sgtf_within)[[2]], 
                  inf_shift=5,
                  logmean_inc=logmean_inc_sgtf, 
                  logsd_inc=logsd_inc_sgtf, 
                  r=r)
  
  mean <- mean_inc_sgtf - 5 + coef(ff)[[2]]
  ...
  data.frame(
    ...
    param=c("inf_shape", "inf_mean", "mean"),
    est=c(coef(ff)[[1]], coef(ff)[[2]], mean),
    lwr=c(cc[1,1], cc[2,1], mean_inc_sgtf - 5 + cc[2,1]),
    upr=c(cc[1,2], cc[2,2], mean_inc_sgtf - 5 + cc[2,2]),
    type="Omicron"
  )
}) %>%
  bind_rows

save("fit_he_r_50_sgtf_within", "fit_he_r_50_nsgtf_within", file="fit_he_r.rda")

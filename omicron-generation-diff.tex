...
\DIFdelbegin \DIFdel{A few studies have }\DIFdelend \DIFaddbegin \DIFadd{Some studies have also }\DIFaddend estimated that the Omicron variant has shorter transmission intervals than the Delta variant \citep{abbott2022test,kremer2022observed,song2022serial}, but there has been a lack of direct generation-interval estimates.
\cite{ito2022estimating,selby2022generation} \DIFdelbegin \DIFdel{tried to estimate }\DIFdelend \DIFaddbegin \DIFadd{estimated }\DIFaddend the generation-interval distributions of the Omicron variant but they both relied on population-level epidemic dynamics (rather than individual-level transmission data).
\DIFdelbegin \DIFdel{Although we }\DIFdelend \DIFaddbegin \DIFadd{Here, we we }\DIFaddend estimate a shorter mean \DIFaddbegin \emph{\DIFadd{realized}} \DIFaddend generation interval for the Omicron variant\DIFdelbegin \DIFdel{, we find the generation-interval distribution of }\DIFdelend \DIFaddbegin \DIFadd{.
...
\DIFadd{Shorter realized generation intervals of the Omicron variant may be driven, in part, by shorter intrinsic generation intervals.
...
However, }\DIFaddend the \DIFdelbegin \DIFdel{Omicron and Delta variants have similar modes (around 2.5 days), implying that the realized transmissibility }\DIFdelend \DIFaddbegin \DIFadd{individual-level relationship between immunity and the intrinsic generation-interval distributions }\DIFaddend of the Omicron variant \DIFdelbegin \DIFdel{decays faster.
We }\DIFdelend \DIFaddbegin \DIFadd{is difficult to predict given its high immune evasiveness:
...
\DIFadd{Even if the Delta and Omicron variants had identical intrinsic generation-interval distributions (e.g., a lack of changes in viral kinetics or the effect of pre-existing immunity), their realized generation-interval distributions could still differ.
...
\DIFadd{Instead, we }\DIFaddend tentatively hypothesize that \DIFdelbegin \DIFdel{these differences }\DIFdelend \DIFaddbegin \DIFadd{the differences in realized generation intervals }\DIFaddend may be primarily driven by the network effect \citep{park2020inferring,hart2022generation}: a higher reproduction number of the Omicron variant leads to faster susceptible depletion among close contacts, which in turn prevents long generation intervals from \DIFdelbegin \DIFdel{generating infections. 
}\DIFdelend \DIFaddbegin \DIFadd{being realized. 
...
}\DIFaddend Previous simulations showed that \DIFaddbegin \DIFadd{such }\DIFaddend network effects can have considerable impact on realized generation intervals \DIFdelbegin \DIFdel{even during the initial exponential growth phase, when susceptible depletion is negligible at the population level }\DIFdelend \citep{park2020inferring}.
...
\DIFdelbegin \DIFdel{Shorter generation-interval estimates for between-household contacts may be attributable to behavioral effects: individuals who have symptoms or tested positive may be more likely to stay home, preventing long between-household transmission.
Other factors, such as more stringent contact tracing measures against the Omicron variant in the Netherlands \mbox{%DIFAUXCMD
\citep{backer2021omicron} }\hspace{0pt}%DIFAUXCMD
faster within-host clearance of the Omicron variant \mbox{%DIFAUXCMD
\citep{hay2022viral}}\hspace{0pt}%DIFAUXCMD
, and viral kinetics of reinfection and breakthrough infections, also likely contributed to shortening of generation intervals.
...
\DIFdelbegin \DIFdel{While our }\DIFdelend \DIFaddbegin \DIFadd{Our }\DIFaddend study indicates that the Omicron variant has a shorter mean realized generation interval than that of the Delta variant, \DIFdelbegin \DIFdel{it is still uncertain how infectiousness profiles differ intrinsically between Omicron and Delta}\DIFdelend \DIFaddbegin \DIFadd{but the difference between the intrinsic infectiousness profiles remains unclear}\DIFaddend .
...
In addition, the \DIFdelbegin \DIFdel{``intrinsic '' generation intervals of both Omicron and Delta }\DIFdelend \DIFaddbegin \DIFadd{counter-factual initial intrinsic generation intervals (for an immunologically naive population) of both }\DIFaddend variants are likely longer than what we estimate given existing levels of interventions, including vaccination, and pandemic awareness---estimating \DIFdelbegin \DIFdel{intrinsic (or ``unmitigated'') }\DIFdelend \DIFaddbegin \DIFadd{initial intrinsic }\DIFaddend generation-interval distributions of SARS-CoV-2 variants is expected to be a difficult problem as it requires data from times when awareness levels were low \citep{sender2021unmitigated}.

...
Incubation period data were originally collected from 513 individuals (consisting of 258 Omicron 165 and 255 Delta cases), with symptom onsets between 1 December 2021 and 2 January 2022;
...
To estimate the growth rates differences of the Delta and Omicron variants, we first estimate the number of COVID-19 cases infected with each variant by multiplying reported weekly numbers of cases by the proportion of Delta and Omicron variants detected---we use weekly time series to smooth over patterns of testing and reporting within each week. 
...
we note that the estimates of the proportion of each variant can be still sensitive to non-randomness of the sampling.
...
More specifically, we model the logged weekly numbers of cases infected with each variant as a function of time using a shrinkage version of a cubic spline with restricted maximum likelihood (specified as \texttt{s(time, bs="cs")} using the \texttt{MGCV} \texttt{R} package).
...
here, we use Gaussian likelihoods on logged cases because the inferred numbers of cases infected with each variant are not counts.
...
We cover a wide range of $\rho$ to consider the possibility that our original assumpton ($\rho=0.75$) may under or over-estimate the true $\rho$.

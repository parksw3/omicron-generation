...
While the network effect is expected to be strongest among household contacts, it is also applicable to other forms of contact structres that involve repeated contacts between the same group of individuals (beacuse only the first infectious contact results in infection).
...
Other factors, such as more stringent intervention measures against the Omicron variant \citep{backer2021omicron} and shoter clearance phase of the Omicron variant \citep{hay2022viral}, also likely contributed to shortening of the generation intervals.
...
In doing so, we implicitly assume thta the delay between infection and reports is fixed.
...
Our study sythesizes previously developed theoretical framework on serial- and generation-interval distributions and presents methodological advances in monitoring epidemiological parameters;

...
%% Then, the instantaneous reproduction number is estimated using the renewal equation \citep{fraser2007estimating}: This looks like an orphan to me  \jd
...
\citep{backer2021omicron} previously reported shorter mean serial interval of the Omicron variant (3.5 days) than that of the Delta variant (4.1 days) for this cohort, but the overall shapes of the distributions are similar (\fref{serial}A).
...
We note that the mean forward generation interval of the Omicron (and Delta) variant is shorter (longer) than its mean serial interval due to the effect of a growing (declining) epidemic.

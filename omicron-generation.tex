...
Incubation period data were originally collected from 513 individuals (consisting of 258 Omicron \jd{What?: 165} and 255 Delta cases), with symptom onsets between 1 December 2021 and 2 January 2022;
...
\jd{I understand why we show the differences, but I think we're doing calculations with the separate growth rates, not the differences and should talk less about differences here.}
...
More specifically, we model the logged weekly numbers of cases infected with each variant as a function of time using a penalized cubic spline fitted with restricted maximum likelihood (specified as \texttt{s(time, bs="cs")} using the \texttt{MGCV} \texttt{R} package). 
\jd{I am not sure if it's good to have the s() statement without the rest of the gam() statement. Should we leave it out?}
...
(B) Generation intervals for the cohort of infectors who are infected at the same time will have unbiased incubation periods. \jd{Awkward; in what sense do GIs ``have'' incubation periods? maybe: are not biased by dynamical effects on incubation periods?}
\label{fig:diagram}
}
\end{figure}
